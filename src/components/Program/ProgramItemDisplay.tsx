...
interface ProgramItemDisplayProps {
    auth: ClowdrState | null;
    id: string;
    showBreakoutRoom: boolean;
}

interface ProgramItemDisplayState {
    ProgramItem: Parse.Object;
    loading: boolean;
}

export default class ProgramItemDisplay extends React.Component<ProgramItemDisplayProps, ProgramItemDisplayState> {
    constructor(props: ProgramItemDisplayProps) {
        super(props);
        this.state = {
            ProgramItem: new Parse.Object, // TS: assigned an empty general Parse object when initialized
            loading: true
        };
    }
    ...
    render() {
        ...
        let authorstr = "";
        let authorsArr = authors.map((a: ProgramPerson) => <ProgramPersonDisplay key={a.id} auth={this.props.auth}
                                                                                 id={a.id}/>);
        if (authorsArr.length >= 1)
            authorstr = authorsArr.reduce((prev: any | string, curr: any | string) => [prev, ", ", curr]);
        let sessionInfo;
        let now = Date.now();
        ...
        return <div className="program-item-display">
            <Tooltip title={tooltip}>
                <div className="text-indented"><NavLink
                    to={"/program/" + this.state.ProgramItem.get("confKey")}>{this.state.ProgramItem.get("title")}</NavLink>
                </div>
            </Tooltip>
            {this.props.showBreakoutRoom && this.state.ProgramItem.get("breakoutRoom") ?
                <BreakoutRoomDisplay id={this.state.ProgramItem.get("breakoutRoom").id}/> : <></>}
        </div>
    }
}